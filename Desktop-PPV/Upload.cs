        ...
        private void btn_Ingresar_Click(object sender, EventArgs e)
        {
            for (int i = 0; i <= dataGridView1.Rows.Count - 1; i++)
            {
                ...
                try
                {
                    ...
                    if (!con.Existe("select count(*) from tb_PN where pn = '" + dataGridView1.Rows[i].Cells[0].Value.ToString() + "'"))
                    {
                        ...
                        //con.Crud("insert into tb_PN values('" + dataGridView1.Rows[i].Cells[0].Value.ToString() + "')");
                        MessageBox.Show(i+"!");
